 ...
 * This null object can be used to express that the result of a request is 'none', as is done in many SKB types.
 * It is build as an Enumerate, so that only one instantiation exists at runtime. This implies that
 * this type should only be used to express a 'none' result, but not to identify a specific 'none' result.
 *
...
public enum NONone implements NullObject {
	/** The only instance for SkbTypeDefNone */
	get;

	@Override
	public Com_CoinType getType() {
		return Com_CoinType.NO_NONE;
	}
